...
class ExploratoryDataAnalysis(Charts, object):
    ...
    def _corr_value_greater(self, corr_threshold):
        ...
        dataframe_corr = super(ExploratoryDataAnalysis, self).correlation_plot(
            self.dataframe
        )
        g = []
        for i in range(dataframe_corr.shape[0]):
            for j in range(dataframe_corr.shape[0]):
                if (dataframe_corr.iloc[i, j] > corr) or (
                    dataframe_corr.iloc[i, j] < -corr
                ):
    ...
    def vif_value(self):
        ...
        ols = statsmodels.regression.linear_model.OLS(
            self.target, self.dataframe.drop(labels=[self.target.name]), axis=1
        )
        res_ols = ols.fit()
