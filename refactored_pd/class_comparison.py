...
class ModelComparison(ModelPerfomance, DecisionTree, LogRegression, object):
    ...
    def cmp_performance_metrics(
        self, ccpalpha, threshold_1, threshold_2, threshold=None
    ):
        ...
        threshold_glm, accuracy_glm, f1_glm, auc_glm = (
            super(ModelComparison, self).glm_perf_analytics(
                self.x_val_glm_n, self.y_val_glm
            )[1],
            super(ModelComparison, self).glm_perf_analytics(
                self.x_val_glm_n, self.y_val_glm
            )[2],
        ...
        (
            ...
            super(ModelPerfomance, self).dt_pruned_perf_analytics(
                ccpalpha,
                threshold_1,
                threshold_2,
                self.x_val_dt,
        ...
        threshold_lg, accuracy_lg, f1_lg, auc_lg = (
            super(DecisionTree, self).lg_perf_analytics(
                self.x_val_lg, self.y_val_lg, threshold
            )[1],
            super(DecisionTree, self).lg_perf_analytics(
                self.x_val_lg, self.y_val_lg, threshold
            )[2],
    ...
    def cmp_overfitting(self, ccpalpha, threshold_1, threshold_2, *thresholds):
        ...
        val_precisions_glm, val_recalls_glm = (
            super(ModelComparison, self).glm_overfitting_test(
                self.x_val_glm_c,
                self.y_val_glm,
                self.x_test_glm_c,
                self.y_test_glm,
        ...
        val_precisions_dt, val_recalls_dt = (
            super(ModelPerfomance, self).dt_pruned_overfitting(
                ccpalpha,
                threshold_1,
                threshold_2,
                self.x_val_dt,
        ...
        val_precisions_lg, val_recalls_lg = (
            super(DecisionTree, self).lg_overfitting_test(
                self.x_train_lg,
                self.y_train_lg,
                self.x_test_lg,
                self.y_test_lg,
    ...
    def cmp_confusion_matrix_plot(
        self, ccpalpha, threshold_1, threshold_2, threshold=None
    ):
        """Confusion matrix for GLM - Logistic Regression"""

        self.x_val_glm_n = sm.add_constant(self.x_val_glm.values, has_constant="add")
        glm_conf_matrix = super(ModelComparison, self).confusion_matrix_plot(
            self.x_val_glm_n, self.y_val_glm
        )[1]

        """ Confusion Matrix for Decision Trees """

        dt_conf_matrix = super(ModelPerfomance, self).dt_pruned_conf_matrix(
            ccpalpha, threshold_1, threshold_2, self.x_val_dt, self.y_val_dt, threshold
        )[1]

        """ Confusion Matrix for LogisticRegression """

        lg_conf_matrix = super(DecisionTree, self).lg_confusion_matrix_plot(
            self.x_val_lg, self.y_val_lg, threshold
        )[1]
        plt.close("all")
        self.fig, (self.axs1, self.axs2, self.axs3) = plt.subplots(1, 3)
        ...
        def generate(axs, conf, title_):
            ...
            axs.set_yticklabels(
                ["", "No Default", "Yes Default"], rotation=90, va="center"
            )
