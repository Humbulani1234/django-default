    ...
    def little_test(dataframe, values):
        ...
        try:           
            ...
            for i in range(len(k)):                                                               
                first_component = (np.array(Observed_Variables_Mean_Cov(dataframe, values)[0][i])\
                 - np.array(Observed_Variables_imputed_mean_cov(dataframe, values)[0][i]))[:,np.newaxis].T
                second_component = np.linalg.inv(np.array(Observed_Variables_Mean_Cov(dataframe, values)[1][i]))
                third_component = (np.array(Observed_Variables_Mean_Cov(dataframe, values)[0][i])\
                                  - np.array(Observed_Variables_imputed_mean_cov(dataframe, values)[0][i]))
                Little_Test.append(k[i]*(np.dot(np.dot(first_component, second_component), third_component)))
