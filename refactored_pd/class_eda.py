...
class ExploratoryDataAnalysis(OneHotEncoding, object):
    ...
    def __init__(
        self,
        custom_rcParams,
        df_nomiss_cat: pd.DataFrame,
        type_,
        df_loan_float,
        target,
        randomstate,
    ):
        super(ExploratoryDataAnalysis, self).__init__(
            custom_rcParams, df_nomiss_cat, type_, randomstate
        )
        self.df_loan_float_eda = df_loan_float
        self.target_eda = target
        self.x_eda = super(ExploratoryDataAnalysis, self).create_xy_frames(
            self.df_loan_float_eda, self.target_eda
        )[0]
        self.y_eda = super(ExploratoryDataAnalysis, self).create_xy_frames(
            self.df_loan_float_eda, self.target_eda
        )[1]
    ...
    def cat_missing_pivot_plot(self, independent, target):
        ...
        df_pivot = pd.pivot_table(
            df, index=independent.name, values=target.name, aggfunc=len, fill_value=0
    ...
    def cat_pivot_plot(self, independent, target):
        ...
        df_pivot = pd.pivot_table(
            df, index=independent.name, columns=target.name, aggfunc=len, fill_value=0
        ).apply(lambda x: x / float(x.sum()))
    ...
    def correlation_plot(self, dataframe):
        ...
        corr = dataframe.corr()
        sns.heatmap(
            corr, annot=True, cmap="coolwarm", fmt=".2f", linewidths=0.5, ax=self.axs
        )
    ...
    def _corr_value_greater(self, dataframe, corr_threshold):
        ...
        for i in range(dataframe_corr.shape[0]):
            for j in range(dataframe_corr.shape[0]):
                if (dataframe_corr.iloc[i, j] > corr_threshold) or (
                    dataframe_corr.iloc[i, j] < -corr_threshold
                ):
    ...
    def vif_value(self, dataframe, target):
        ...
        ols = sm.regression.linear_model.OLS(
            target, dataframe.drop(labels=[self.target.name]), axis=1
        )
        res_ols = ols.fit()
    ...
    def point_biserial(self):
        ...
        proportion_GB_1 = (
            df_1[self.target.name].count()
            / df_loan_float_po_bi[self.target.name].count()
        )
        proportion_GB_0 = 1 - proportion_GB_1
        r_po_bi = ((j - k) / standard_dev_AGE) * sqrt(
            (self.independent.shape[0] * proportion_GB_0)
            * (1 - proportion_GB_0)
            / (self.independent.shape[0] - 1)
        )
...
if __name__ == "__main__":
    ...
    print(
        c.scatter_plot(
            df_loan_float["CHILDREN"], df_loan_float["GB"], df_loan_float["AGE"]
        )
    )
