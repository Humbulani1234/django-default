...
def confusion_decision(request):
    f = data.d.dt_pruned_conf_matrix(
        data.ccpalpha,
        data.threshold_1,
        data.threshold_2,
        data.d.x_test_dt,
    ...
    if cached_result is not None:
        render(
            request,
            "decision/peformance/confusion_decision.html",
...
def decision_tree(request):
    ...
    if cached_result is not None:
        render(
            request,
            "decision/peformance/decision_tree.html",
            {"image_base64": cached_result},
        )
    image_base64 = image_generator(f)
    cache.set(cache_key, image_base64, 3600)
...
def cross_validate(request):
    ...
    if cached_result is not None:
        render(
            request,
            "decision/peformance/cross_validate.html",
            {"image_base64": cached_result},
        )
    image_base64 = image_generator(f)
    cache.set(cache_key, image_base64, 3600)
...
def confusion_cmp(request):
    f = data.o.cmp_confusion_matrix_plot(
        data.ccpalpha, data.threshold_1, data.threshold_2, data.threshold
    )
    ...
    if cached_result is not None:
        render(
            request,
            "decision/comparison/confusion_cmp.html",
            {"image_base64": cached_result},
        )
    image_base64 = image_generator(f)
    cache.set(cache_key, image_base64, 3600)
...
def overfitting_cmp(request):
    f = data.o.cmp_overfitting(
        data.ccpalpha, data.threshold_1, data.threshold_2, *np.arange(0.1, 0.9, 0.05)
    )
    ...
    if cached_result is not None:
        render(
            request,
            "decision/comparison/overfitting.html",
            {"image_base64": cached_result},
        )
    image_base64 = image_generator(f)
    cache.set(cache_key, image_base64, 3600)
    image_base64 = image_generator(f)
    return render(
        request, "decision/comparison/overfitting.html", {"image_base64": image_base64}
    )


def perf_analytics_cmp(request):
    f = data.o.cmp_performance_metrics(
        data.ccpalpha, data.threshold_1, data.threshold_2, data.threshold
    )
    ...
    if cached_result is not None:
        render(
            request,
            "decision/comparison/perf_analytics.html",
            {"image_base64": cached_result},
        )
    image_base64 = image_generator(f)
    cache.set(cache_key, image_base64, 3600)
...
def tree(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            Radio_TV_Hifi, Furniture_Carpet, Dept_Store_Mail, Leisure, Cars, OT = (
                0,
                0,
                0,
                0,
            ...
            answer = data.d.dt_sample_pruned_prob(
                data.ccpalpha, data.threshold_1, data.threshold_2, data.sample, inputs
            )
            try:
                with transaction.atomic():
                    dec_features_object = DecFeatures.objects.get(pk=saved_pk)
                    probability_instance = DecProbability(
                        CUSTOMER_ID=dec_features_object
                    )
                    probability_instance.probability = answer
                    probability_instance.default = (
                        "default" if answer > 0.47 else "nodefault"
                    )
        ...
        form = Inputs()
    return render(
        request, "decision/model/decision.html", {"form": form, "answer": answer}
    )
